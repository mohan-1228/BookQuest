...
import { booksAPI, requestsAPI, quotesAPI } from "../services/api";
import {
  BookOpen,
  Search,
  Bell,
...
const UserDashboard = () => {
  ...
  const [myQuotes, setMyQuotes] = useState([]); // Quotes on user's requests
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch user's requests and quotes for those requests
        const [requestsResponse, quotesResponse] = await Promise.all([
          requestsAPI.getMyRequests(), // User's own requests
          quotesAPI.getUserQuotes(), // Quotes on user's requests
        ]);

        setMyRequests(requestsResponse.data.data || []);
        setMyQuotes(quotesResponse.data.data || []);
  ...
  const handleAcceptQuote = async (quoteId) => {
    try {
      ...
      // Refresh quotes data
      const quotesResponse = await quotesAPI.getUserQuotes();
      setMyQuotes(quotesResponse.data.data || []);
    } catch (error) {
...
//               <div key={quote._id} className="p-4 border rounded-lg hover:bg-gray-50">
//                 <div className="flex justify-between items-start">
//                   <div>
//                     <h4 className="font-medium">{quote.requestId?.title}</h4>
//                     <p className="text-sm text-gray-500">Vendor: {quote.vendorId?.businessName}</p>
//                     <p className="text-sm text-gray-500">Price: ${quote.price}</p>
...
//                 </div>
//                 {quote.notes && (
//                   <p className="text-sm text-gray-500 mt-2">Notes: {quote.notes}</p>
//                 )}
//               </div>
//             ))}
//           </div>
