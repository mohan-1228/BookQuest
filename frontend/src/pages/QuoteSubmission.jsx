...
const SubmitQuote = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { contactPerson, businessName, email, phone, address } =
        formData.vendorDetails;
      if (!contactPerson || !businessName || !email || !phone || !address) {
        throw new Error("Please fill in all required vendor details");
      }

      // Validate books have unit prices
      const hasEmptyPrices = formData.books.some(
        (book) => !book.unitPrice || parseFloat(book.unitPrice) <= 0
      );
      if (hasEmptyPrices) {
      ...
      await quotesAPI.createQuotes(id, {
        vendorDetails: formData.vendorDetails, // Make sure this is included
        books: formData.books.map((book) => ({
          bookId: book.bookId,
          title: book.title,
          author: book.author,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0B2E33] to-[#14464b] py-8 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Vendor Details Form - NOW AT THE TOP */}
          <div className="bg-white rounded-xl p-6 text-black">
            <h2 className="text-xl font-semibold mb-4 flex items-center">
              <User className="h-5 w-5 mr-2" />
              Vendor Information
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full p-3 border border-gray-900 rounded-lg focus:ring-2 focus:ring-[#0B2E33] focus:border-[#0B2E33] bg-white text-black"
                  required
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full p-3 border border-gray-900 rounded-lg focus:ring-2 focus:ring-[#0B2E33] focus:border-[#0B2E33]"
                  required
                />
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full p-3 rounded-lg bg-white border border-gray-300 focus:ring-2 focus:ring-[#0B2E33] focus:border-[#0B2E33] text-gray-900 appearance-none"
                  min={new Date().toISOString().split("T")[0]}
                />
                <div className="absolute right-3 top-9 pointer-events-none">
                  <svg
          ...
          <div className="flex justify-end space-x-4 pt-6">
            <Link
              to={`/requests/${id}`}
              className="px-6 py-3 bg-[#7d0707] border border-gray-300 rounded-lg text-gray-900 hover:bg-gray-50 font-medium"
            >
              Cancel
            </Link>
            <button
              type="submit"
              disabled={submitting || totalPrice <= 0}
              className="px-8 py-3 bg-[#11963d] text-gray-900 rounded-lg hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed flex items-center font-medium"
            >
              {submitting ? (
                <>
                  <Loader className="h-4 w-4 animate-spin mr-2" />
